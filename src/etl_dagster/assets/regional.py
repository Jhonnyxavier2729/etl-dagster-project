...
@asset(group_name="regional")
def clean_data_regional(
    context: AssetExecutionContext, raw_data_regional: pd.DataFrame
):
...
@asset(group_name="regional")
def clean_csv_data_regional(
    context: AssetExecutionContext, clean_data_regional: pd.DataFrame
):
...
@asset(group_name="regional", required_resource_keys={"mongodb_resource"})
def mongodb_data_regional(
    context: AssetExecutionContext,
    clean_data_regional: pd.DataFrame,
):
...
@asset(group_name="regional", required_resource_keys={"postgres_resource"})
def postgres_data_regional(
    context: AssetExecutionContext,
    clean_data_regional: pd.DataFrame,
):
    ...
    try:
        ...
        # ✅ Cargar datos
        clean_data_regional.to_sql(
            "regional_limpios", engine, if_exists="replace", index=False
        )
...
@asset(group_name="regional", required_resource_keys={"duckdb_resource"}, deps=[duckdb_data_ofertas])
def duckdb_data_regional(
    context: AssetExecutionContext,
    clean_data_regional: pd.DataFrame,
):
    ...
    try:
        ...
        with context.resources.duckdb_resource.get_connection() as con:
            ...
            con.execute(
                "CREATE OR REPLACE TABLE regional_limpios AS SELECT * FROM temp_clean_data"
            )
